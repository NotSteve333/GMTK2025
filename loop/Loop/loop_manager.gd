...
func _process(_delta):
	if enabled:
		...
		if !head.is_stopped and ((trail_interior.get_point_count() < 1) or head_pos.distance_to(trail_interior.get_point_position(num_nodes - 1)) > 7.0):
...
func push_fuck(delta: float) -> void:
	for j in range(num_nodes):
		var diff = trail_border.get_point_position(j) - active_inter.center
		var diff_m = diff.length()
		trail_border.set_point_position(j, trail_border.get_point_position(j).move_toward(trail_border.get_point_position(j) + diff, delta * min(diff_m, max_pull_speed)))
		trail_interior.set_point_position(j, trail_border.get_point_position(j))
